<!--
THE ORIGINAL PROJECT README CONTENT IS AUTOMATICALLY GENERATED.
...
## Souitou-iop fork: Mac Mouse Fix 3.1.0

This repository is a community-maintained fork of [noah-nuebling/mac-mouse-fix](https://github.com/noah-nuebling/mac-mouse-fix). It keeps the original project's functionality and licensing system while integrating focused compatibility, per-app scrolling, stability, localization, and UI improvements.

| | Current status |
|---|---|
| Version | **3.1.0 (24832)** |
| Architecture | **Apple Silicon (`arm64`)** for locally packaged builds |
| Minimum deployment target | **macOS 12** |
| Stable branch | [`main`](https://github.com/Souitou-iop/mac-mouse-fix/tree/main) |
| Development branch | [`dev`](https://github.com/Souitou-iop/mac-mouse-fix/tree/dev) |

### Changes in this fork

- **Per-app smooth scrolling exclusions.** The new `Apps` tab lets you select one or more applications that should use native, non-smooth scrolling. Other applications continue to use the global scrolling configuration.
- **macOS 27 dock-swipe compatibility.** Spaces, Mission Control, App Exposé, Show Desktop, and related modified-drag gestures use an augmented HID event path, including safer multi-display gesture termination.
- **Helper stability.** Foreground-app configuration changes and derived UI state are applied on the main thread, preventing the Helper from aborting and being relaunched when AppKit state was previously changed from a scrolling worker queue.
- **Native tab transitions.** The `Apps` tab follows the same top-anchored spring resize and outgoing-page fade used by the original tabs, without dark double-fade frames or oversized windows.
- **Localized Apps UI.** The new interface includes English, Simplified Chinese, and Traditional Chinese strings, with English fallback for other languages.

### How app exclusions work

Adding an app writes only a `Scroll.smooth = off` override for that application. Scroll speed, direction, modifiers, button mappings, and other settings continue to inherit the global configuration. Removing an app removes only that smooth-scrolling override and preserves any unrelated per-app configuration.

Rules are selected from the current foreground application. Apps without a bundle identifier are tracked by their normalized application path. The Helper reloads the merged configuration immediately when the foreground app changes.

This is a focused smooth-scrolling exclusion list, not a general-purpose profile editor. Per-app button mappings, scroll speed profiles, and mouse-specific profiles are not included.

### Installation and updates

The Helper is embedded inside `Mac Mouse Fix.app`; do not install `Mac Mouse Fix Helper.app` separately. For a locally built package, move the complete main app bundle to `/Applications`, then grant the requested Accessibility permissions.

This fork currently maintains source branches rather than an independently notarized GitHub release. The website, Homebrew, and upstream release links in the original README below install the original project, not this fork. Locally produced ad-hoc builds are intended for personal testing.

The upstream Sparkle update feed is intentionally unchanged. Accepting a future upstream update may replace this fork build and remove fork-specific features.

### Build from source

Open `Mouse Fix.xcodeproj`, select the `App - Release` scheme, choose an Apple Silicon Mac destination, and configure your own signing team. The release build used for this fork is restricted to `arm64`; no x86_64 package is produced.

Command-line compilation can use the following baseline:
...
See the [Chinese README](README.zh-CN.md) for a complete Chinese overview. This fork remains derived from Mac Mouse Fix and is distributed under the original [MMF License](License).
...
- **Are there 'App-Specific Settings' or 'Profiles'?**
  ...
  This fork includes a focused app-specific setting for smooth scrolling. Open the `Apps` tab and add any application that should use native, non-smooth scrolling. Remove it from the list to restore the global smooth-scrolling setting.

  This does not provide general profiles. Scroll speed, direction, button mappings, and other settings still inherit the global configuration, and mouse-specific profiles are not included.
...
- **Why do the 'Scrolling' and 'Buttons' items in the Menu Bar get re-enabled?**
  ...
  For a permanent per-app smooth-scrolling exclusion, use the `Apps` tab. Other settings remain global. <br>
  ...
  The menu-bar controls remain useful for temporary global changes, while the `Apps` tab stores persistent smooth-scrolling exclusions.
-->
